...
struct verbs_device {
  ...
  int (*init_context)(struct verbs_device *device, struct ibv_context *ctx,
                      int cmd_fd);
...
gcclResult_t wrap_ibv_fork_init(void);
...
gcclResult_t wrap_ibv_open_device(struct ibv_context **ret,
                                  struct ibv_device *device);
gcclResult_t wrap_ibv_close_device(struct ibv_context *context);
gcclResult_t wrap_ibv_get_async_event(struct ibv_context *context,
                                      struct ibv_async_event *event);
gcclResult_t wrap_ibv_ack_async_event(struct ibv_async_event *event);
gcclResult_t wrap_ibv_query_device(struct ibv_context *context,
                                   struct ibv_device_attr *device_attr);
gcclResult_t wrap_ibv_query_port(struct ibv_context *context, uint8_t port_num,
                                 struct ibv_port_attr *port_attr);
...
gcclResult_t wrap_ibv_alloc_pd(struct ibv_pd **ret,
                               struct ibv_context *context);
gcclResult_t wrap_ibv_dealloc_pd(struct ibv_pd *pd);
...
struct ibv_mr *wrap_direct_ibv_reg_mr(struct ibv_pd *pd, void *addr,
                                      size_t length, int access);
gcclResult_t wrap_ibv_dereg_mr(struct ibv_mr *mr);
gcclResult_t wrap_ibv_create_comp_channel(struct ibv_comp_channel **ret,
                                          struct ibv_context *context);
gcclResult_t wrap_ibv_destroy_comp_channel(struct ibv_comp_channel *channel);
...
static inline gcclResult_t wrap_ibv_poll_cq(struct ibv_cq *cq, int num_entries,
  ...
  if (done < 0) {
    LOG(FATAL) << "Call to ibv_poll_cq() returned " << done;
  }
  *num_done = done;
  return gcclSuccess;
}
...
gcclResult_t wrap_ibv_modify_qp(struct ibv_qp *qp, struct ibv_qp_attr *attr,
                                int attr_mask);
...
static inline gcclResult_t wrap_ibv_post_send(struct ibv_qp *qp,
  ...
  if (ret != IBV_SUCCESS) {
    LOG(FATAL) << "ibv_post_send() failed with error " << strerror(ret);
    return gcclSystemError;
  }
  return gcclSuccess;
}

static inline gcclResult_t wrap_ibv_post_recv(struct ibv_qp *qp,
  ...
  int ret = qp->context->ops.post_recv(
      qp, wr, bad_wr); /*returns 0 on success, or the value of errno on failure
                          (which indicates the failure reason)*/
  if (ret != IBV_SUCCESS) {
    LOG(FATAL) << "ibv_post_recv() failed with error " << strerror(ret);
  }
  return gcclSuccess;
}
