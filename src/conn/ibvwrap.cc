...
static enum {
  ...
  ibvInitialized,
  ibvError
} ibvState = ibvUninitialized;  // OK to compile
...
int (*ibv_internal_get_async_event)(struct ibv_context *context,
                                    struct ibv_async_event *event);
void (*ibv_internal_ack_async_event)(struct ibv_async_event *event);
int (*ibv_internal_query_device)(struct ibv_context *context,
...
struct ibv_mr *(*ibv_internal_reg_mr)(struct ibv_pd *pd, void *addr,
                                      size_t length, int access);
int (*ibv_internal_dereg_mr)(struct ibv_mr *mr);
struct ibv_cq *(*ibv_internal_create_cq)(struct ibv_context *context, int cqe,
                                         void *cq_context,
                                         struct ibv_comp_channel *channel,
                                         int comp_vector);
int (*ibv_internal_destroy_cq)(struct ibv_cq *cq);
struct ibv_qp *(*ibv_internal_create_qp)(struct ibv_pd *pd,
                                         struct ibv_qp_init_attr *qp_init_attr);
int (*ibv_internal_modify_qp)(struct ibv_qp *qp, struct ibv_qp_attr *attr,
                              int attr_mask);
...
gcclResult_t wrap_ibv_symbols(void) {
  if (ibvState == ibvInitialized) return gcclSuccess;
  if (ibvState == ibvError) return gcclSystemError;

  if (__sync_bool_compare_and_swap(&ibvState, ibvUninitialized,
                                   ibvInitializing) == false) {
    // Another thread raced in front of us. Wait for it to be done.
    while (ibvState == ibvInitializing) pthread_yield();
    return (ibvState == ibvInitialized) ? gcclSuccess : gcclSystemError;
  }
...
#define LOAD_SYM(handle, symbol, funcptr)                                 \
  do {                                                                    \
    ...
    if (tmp == NULL) {                                                    \
      LOG(WARNING) << "dlvsym failed on " << symbol << " - " << dlerror() \
                   << " version " << IBVERBS_VERSION;                     \
      goto teardown;                                                      \
    }                                                                     \
    *cast = tmp;                                                          \
  } while (0)
...
#define IBV_PTR_CHECK_ERRNO(name_internal, call, retval, error_retval, name) \
  ...
  if (retval == error_retval) {                                              \
    LOG(FATAL) << "Call to " << name << " failed with error "                \
               << strerror(errno);                                           \
  }                                                                          \
  return gcclSuccess;
...
gcclResult_t wrap_ibv_fork_init() {
  IBV_INT_CHECK(ibv_internal_fork_init, ibv_internal_fork_init(), -1,
                "ibv_fork_init");
}

gcclResult_t wrap_ibv_get_device_list(struct ibv_device ***ret,
                                      int *num_devices) {
...
gcclResult_t wrap_ibv_free_device_list(struct ibv_device **list) {
  IBV_PASSTHRU(ibv_internal_free_device_list,
...
gcclResult_t wrap_ibv_open_device(
    struct ibv_context **ret,
    struct ibv_device *device) { /*returns 0 on success, -1 on failure*/
  IBV_PTR_CHECK(ibv_internal_open_device, ibv_internal_open_device(device),
                *ret, NULL, "ibv_open_device");
}

gcclResult_t wrap_ibv_close_device(
    struct ibv_context *context) { /*returns 0 on success, -1 on failure*/
  IBV_INT_CHECK(ibv_internal_close_device, ibv_internal_close_device(context),
                -1, "ibv_close_device");
}

gcclResult_t wrap_ibv_get_async_event(
    struct ibv_context *context,
    struct ibv_async_event *event) { /*returns 0 on success, and -1 on error*/
  IBV_INT_CHECK(ibv_internal_get_async_event,
                ibv_internal_get_async_event(context, event), -1,
                "ibv_get_async_event");
}

gcclResult_t wrap_ibv_ack_async_event(struct ibv_async_event *event) {
  IBV_PASSTHRU(ibv_internal_ack_async_event,
               ibv_internal_ack_async_event(event));
}

gcclResult_t wrap_ibv_query_device(
    struct ibv_context *context,
    struct ibv_device_attr
        *device_attr) { /*returns 0 on success, or the value of errno on failure
                           (which indicates the failure reason)*/
  IBV_INT_CHECK_RET_ERRNO(ibv_internal_query_device,
                          ibv_internal_query_device(context, device_attr), 0,
                          "ibv_query_device");
}

gcclResult_t wrap_ibv_query_port(
    struct ibv_context *context, uint8_t port_num,
    struct ibv_port_attr
        *port_attr) { /*returns 0 on success, or the value of errno on failure
                         (which indicates the failure reason)*/
  IBV_INT_CHECK_RET_ERRNO(ibv_internal_query_port,
                          ibv_internal_query_port(context, port_num, port_attr),
                          0, "ibv_query_port");
}

gcclResult_t wrap_ibv_query_gid(struct ibv_context *context, uint8_t port_num,
                                int index, union ibv_gid *gid) {
...
gcclResult_t wrap_ibv_query_qp(struct ibv_qp *qp, struct ibv_qp_attr *attr,
                               int attr_mask,
                               struct ibv_qp_init_attr *init_attr) {
  IBV_INT_CHECK_RET_ERRNO(ibv_internal_query_qp,
                          ibv_internal_query_qp(qp, attr, attr_mask, init_attr),
                          0, "ibv_query_qp");
}

gcclResult_t wrap_ibv_alloc_pd(struct ibv_pd **ret,
                               struct ibv_context *context) {
  IBV_PTR_CHECK(ibv_internal_alloc_pd, ibv_internal_alloc_pd(context), *ret,
                NULL, "ibv_alloc_pd");
}

gcclResult_t wrap_ibv_dealloc_pd(
    struct ibv_pd *pd) { /*returns 0 on success, or the value of errno on
                            failure (which indicates the failure reason)*/
  IBV_INT_CHECK_RET_ERRNO(ibv_internal_dealloc_pd, ibv_internal_dealloc_pd(pd),
                          0, "ibv_dealloc_pd");
}

gcclResult_t wrap_ibv_reg_mr(struct ibv_mr **ret, struct ibv_pd *pd, void *addr,
                             size_t length, int access) {
  IBV_PTR_CHECK(ibv_internal_reg_mr,
...
gcclResult_t wrap_ibv_dereg_mr(
    struct ibv_mr *mr) { /*returns 0 on success, or the value of errno on
                            failure (which indicates the failure reason)*/
  IBV_INT_CHECK_RET_ERRNO(ibv_internal_dereg_mr, ibv_internal_dereg_mr(mr), 0,
                          "ibv_dereg_mr");
}

gcclResult_t wrap_ibv_create_cq(struct ibv_cq **ret,
                                struct ibv_context *context, int cqe,
                                void *cq_context,
                                struct ibv_comp_channel *channel,
                                int comp_vector) {
  IBV_PTR_CHECK(
      ibv_internal_create_cq,
      ibv_internal_create_cq(context, cqe, cq_context, channel, comp_vector),
      *ret, NULL, "ibv_create_cq");
}

gcclResult_t wrap_ibv_destroy_cq(struct ibv_cq *cq) {
  IBV_INT_CHECK_RET_ERRNO(ibv_internal_destroy_cq, ibv_internal_destroy_cq(cq),
                          0, "ibv_destroy_cq");
}

gcclResult_t wrap_ibv_destroy_qp(struct ibv_qp *qp) {
  IBV_INT_CHECK_RET_ERRNO(ibv_internal_destroy_qp, ibv_internal_destroy_qp(qp),
                          0, "ibv_destroy_qp");
}

gcclResult_t wrap_ibv_create_qp(struct ibv_qp **ret, struct ibv_pd *pd,
                                struct ibv_qp_init_attr *qp_init_attr) {
  IBV_PTR_CHECK(ibv_internal_create_qp,
                ibv_internal_create_qp(pd, qp_init_attr), *ret, NULL,
                "ibv_create_qp");
}

gcclResult_t wrap_ibv_modify_qp(
    struct ibv_qp *qp, struct ibv_qp_attr *attr,
    int attr_mask) { /*returns 0 on success, or the value of errno on failure
                        (which indicates the failure reason)*/
  IBV_INT_CHECK_RET_ERRNO(ibv_internal_modify_qp,
                          ibv_internal_modify_qp(qp, attr, attr_mask), 0,
                          "ibv_modify_qp");
}

gcclResult_t wrap_ibv_event_type_str(char **ret, enum ibv_event_type event) {
